...
enum UnpackResult unpack_landed_heartbeat(const uint8_t *buf, uint32_t len, struct LandedHeartbeatStats *stats) {
...
enum UnpackResult unpack_flight_heartbeat(const uint8_t *buf, uint32_t len, struct FlightHeartbeatStats *stats) {
...
int pack_command_response(const struct CommandResponse *cmd, uint8_t *buf) {
    ...
    case Command_ImageMetadata:
        return 1 + pack_image_metadata(&cmd->image_metadata, buf + 1);
    default:
        return 1;
    }
    return 1;
...
int pack_image_metadata(const struct ImageMetadata *target, uint8_t *buf) {
    buf[0] = target->image_id;
    uint32_t offset = 1;
    offset += pack_uint32(target->timestamp, &buf[offset]);
    offset += pack_uint16(target->num_blocks, &buf[offset]);
    offset += pack_arm_target(&target->location, &buf[offset]);
    offset += pack_phototransform(&target->transform, &buf[offset]);
    offset += pack_float(target->latitude, &buf[offset]);
    offset += pack_float(target->longitude, &buf[offset]);
    return offset;
}
enum UnpackResult unpack_image_metadata(const uint8_t *buf, uint32_t len, struct ImageMetadata *target) {
    if (len < SIZEOF_PACKED_IMAGE_METADATA) {
        return UnpackResult_TooShort;
    }
    target->image_id = buf[0];
    uint32_t offset = 1;
    unpack_uint32(buf + offset, len - offset, &target->timestamp);
    offset += 4;
    unpack_uint16(buf + offset, len - offset, &target->num_blocks);
    offset += 2;
    unpack_arm_target(buf + offset, len - offset, &target->location);
    offset += SIZEOF_PACKED_ARM_TARGET;
    unpack_phototransform(buf + offset, len - offset, &target->transform);
    offset += SIZEOF_PACKED_PHOTOTRANSFORM;
    unpack_float(buf + offset, len - offset, &target->latitude);
    offset += sizeof(float);
    unpack_float(buf + offset, len - offset, &target->longitude);
    offset += sizeof(float);
    return UnpackResult_AllGood;
}
