...
bool Ina260Driver::writeDefaultConfig() {
  // Continuous voltage + current, 1100 µs conv time on each, 4-sample averaging.
  // Layout: [15:12]=0110 (fixed) | [11:9]=AVG | [8:6]=VBUS_CT | [5:3]=ISH_CT | [2:0]=MODE
  ...
  constexpr uint16_t conf = fixed_top | avg_4 | vbus_ct | ish_ct | mode_cont_both;
...
std::optional<PowerReading> Ina260Driver::read() {
  ...
  if (!readReg16(REG_VBUS, v_raw))    return std::nullopt;
  if (!readReg16(REG_CURRENT, i_raw)) return std::nullopt;
  if (!readReg16(REG_POWER, p_raw))   return std::nullopt;
