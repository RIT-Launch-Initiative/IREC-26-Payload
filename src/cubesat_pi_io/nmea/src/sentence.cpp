...
sentence::sentence(const std::string &nmea_string) {
  ...
  // Talker, assuming the last 3 characters of the tag field are the message
  // type. NOTE: This allows for talkers that are greater than 2 characters.
  ...
  while (delimeter_index != std::string::npos) {
    ...
    std::size_t next_delimiter_index =
        nmea_string.find_first_of(',', delimeter_index + 1);
    ...
    sentence::m_fields.push_back(
        nmea_string.substr(delimeter_index + 1, read_length));

    // Update delimeter index.
    delimeter_index = next_delimiter_index;
  }
}
sentence::sentence(const std::string &talker, const std::string &type,
                   uint8_t n_fields)
...
bool sentence::validate(const std::string &nmea_string) {
  ...
  std::string expected_checksum =
      sentence::checksum(nmea_string, checksum_index);

  // Compare to actual checksum.
  return nmea_string.compare(checksum_index + 1, 2, expected_checksum, 0, 2) ==
         0;
}
std::string sentence::checksum(const std::string &nmea_string,
...
std::string sentence::nmea_string(bool encapsulated) const {
  ...
  // Write fields.
  for (auto field = sentence::m_fields.cbegin();
       field != sentence::m_fields.cend(); ++field) {
    nmea_string.append(",");
