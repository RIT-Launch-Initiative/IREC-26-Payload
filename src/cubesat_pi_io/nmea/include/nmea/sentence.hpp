...
public:
  ...
  sentence(const std::string &talker, const std::string &type,
           uint8_t n_fields = 0);
  ...
  /// \returns The value of the field, or empty string if the field does not
  /// exist.
  ...
  /// \param encapsulated Indicates if this sentence is an encapsulated type,
  /// and should start with '!' instead of '$'.
...
private:
  ...
  /// \param checksum_index The index of the checksum delimeter (*) in the
  /// string.
  /// \returns The 2-character hex checksum.
  static std::string checksum(const std::string &nmea_string,
